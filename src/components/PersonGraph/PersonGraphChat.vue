  ...
  <div class="w-full max-w-4xl mx-auto p-4">
    ...
    <div class="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
      ...
      <div class="flex flex-wrap gap-4 text-sm text-blue-700">
        ...
        <div class="flex items-center">
          <span class="w-2 h-2 rounded-full mr-2" :class="sessionStore.personGraphJson ? 'bg-green-500' : 'bg-gray-400'"></span>
          人物關係圖 ({{ sessionStore.personGraphJson ? '已載入' : '無' }})
        </div>
        <div class="flex items-center">
...
onMounted(() => {
  // 如果有現有的人物關係圖，添加歡迎消息
  if (sessionStore.personGraphJson) {
...
async function sendMessage() {
  ...
  try {
    const response = await fetch('/api/PersonGraphChat', {
      ...
      body: JSON.stringify({
        message: message,
        currentGraph: sessionStore.personGraphJson,
    ...
    if (newGraphJson) {
      try {
        ...
        sessionStore.personGraphJson = newGraphJson
      } catch (e) {
        console.error('接收到的 JSON 格式錯誤:', newGraphJson)
        chatHistory.value.push({
          role: 'assistant',
          content: '抱歉，生成的人物關係圖格式有誤，請重試。',
          timestamp: new Date()
        })
      }
    }
